...
export function EditDepositButton({ booking }: { booking: DepositEditData }) {
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-black/30 px-4 py-8" onClick={() => setOpen(false)}>
      <div className="my-auto w-full max-w-lg rounded-2xl border border-[var(--line)] bg-[var(--card)] p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <form action={updateDeposit} onSubmit={() => setOpen(false)} className="grid gap-3 text-sm sm:grid-cols-2">
          ...
          <label>主人姓名<input name="owner_name" defaultValue={booking.owner_name || ""} className={input}/></label>
          <label>電話 / WhatsApp<input name="contact" defaultValue={booking.contact || ""} className={input}/></label>
          <label>毛孩名字<input name="pet_name" defaultValue={booking.pet_name || ""} className={input}/></label>
          <label>種類<input name="pet_type" defaultValue={booking.pet_type || ""} className={input}/></label>
          <label>服務日期<input type="date" name="service_date" defaultValue={booking.service_date || ""} className={input}/></label>
          <label>服務時間<input type="time" name="service_time" defaultValue={booking.service_time?.slice(0,5) || ""} className={input}/></label>
          <label>狀態<select name="status" defaultValue={booking.status} className={input}>{statuses.map(([v,l]) => <option key={v} value={v}>{l}</option>)}</select></label>
          <label>接送地址<input name="pickup_address" defaultValue={booking.pickup_address || ""} className={input}/></label>
          <label className="sm:col-span-2">備註<textarea name="notes" rows={3} defaultValue={booking.notes || ""} className={input}/></label>
